...
function init() {
  ...
  if (playerName == null) {
    ...
    playerName = prompt('Ingresa tu apodo: ', defaultName);
    if (playerName === null || playerName === "") {
      playerName = defaultName;
    } else {
      setCookie('Nombre', playerName, 24 * 3600);
    }
  }
...
socket.on('responseRoom', function ([name, people, maxPeople]) {
  if (name !== 'error') {
    ...
    dialog(name + ': Esperando jugadores (' + people +'/' + maxPeople + ')');
  } else {
    socket.disconnect();
    alert('Sala llena espera');
  }
});
...
function chooseColor() {
  ...
  ctx.fillStyle = 'black';
  ctx.textAlign = 'center';
  ctx.fillText("Elige un color", canvas.width / 2, canvas.height / 2 - r - 10);
  ctx.textAlign = 'start';
}
